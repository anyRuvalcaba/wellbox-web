...
// El cierre es a las 6pm (hora de México, UTC-6 fijo, sin horario de verano) del día
// anterior a la entrega. En UTC eso es siempre las 00:00 del propio día de entrega —
// por eso getCutoff no necesita saber en qué zona horaria corre el servidor.
describe("getCutoff", () => {
  it("el cierre del 10 de julio es las 00:00 UTC del 10 de julio (18:00 del 9 en México)", () => {
    const cierre = getCutoff("2026-07-10");
    expect(cierre.toISOString()).toBe("2026-07-10T00:00:00.000Z");
  });
  ...
  it("no se mueve con el horario de verano de EE.UU. — México no lo usa desde 2022", () => {
    ...
    expect(getCutoff("2026-01-15").toISOString()).toBe("2026-01-15T00:00:00.000Z");
    expect(getCutoff("2026-07-15").toISOString()).toBe("2026-07-15T00:00:00.000Z");
  });
});

describe("isOrderable", () => {
  it("todavía se puede pedir un minuto antes del cierre", () => {
    const unMinutoAntes = new Date("2026-07-09T23:59:00.000Z");
  ...
  it("ya no se puede pedir justo en el instante del cierre", () => {
    ...
    const momentoExacto = new Date("2026-07-10T00:00:00.000Z");
    expect(isOrderable("2026-07-10", momentoExacto)).toBe(false);
  });

  it("ya no se puede pedir un minuto después del cierre", () => {
    const unMinutoDespues = new Date("2026-07-10T00:01:00.000Z");
...
describe("nextUpcomingCutoff", () => {
  it("de varios días, regresa el cierre más próximo que todavía no pasó", () => {
    ...
    expect(nextUpcomingCutoff(dias, ahora)?.toISOString()).toBe("2026-07-08T00:00:00.000Z");
  });
